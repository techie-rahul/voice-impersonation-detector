...
`speaker_id` must already be enrolled via `POST /enroll`. Example:
`ws://<host>/ws/analyze-call/rahul`

If the speaker is not enrolled the server sends a `SPEAKER_NOT_FOUND` error and
closes the socket.
...
```json
{ "type": "started", "speaker_id": "rahul", "window_seconds": 4.0, "sample_rate": 16000 }
```
...
* `speaker_match` — raw Resemblyzer similarity (higher = same speaker).
